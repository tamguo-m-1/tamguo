...
var vm = new Vue({
	...
	methods: {
		...
		getCouseList: function(){
			return axios.get(mainHttp + "course/findBySubjectId.html?subjectId="+vm.question.subjectId);
		},
		...
		getQuestion : function(questionId){
			return axios.get(mainHttp + "question/info/"+questionId+".html");
		},
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: mainHttp + "question/delete.html",
		...
		saveOrUpdate: function (event) {
			var url = vm.question.uid == null ? mainHttp + "question/save.html" : mainHttp + "question/update.html";
		...
		audit: function(){
			...
			confirm('确定要审核通过选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: mainHttp + "question/audit.html",
				    data: JSON.stringify(questionIds),
				    success: function(r){
				    	if(r.code === 0){
							alert('操作成功', function(index){
		...
		notAudit: function(){
			...
			confirm('确定要审核拒绝选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: mainHttp + "question/notAudit.html",
				    data: JSON.stringify(questionIds),
				    success: function(r){
				    	if(r.code === 0){
							alert('操作成功', function(index){
