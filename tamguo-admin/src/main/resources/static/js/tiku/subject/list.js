$(function () {
    $("#jqGrid").jqGrid({
        url: mainHttp + 'subject/list.html',
        datatype: "json",
        colModel: [		
            { label: '考试ID', name: 'uid', width: 45, key: true , hidden:true},       	
	...
	methods: {
		...
		del:function(event){
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: mainHttp + "subject/delete.html",
		...
		saveOrUpdate:function(event){
			var url = vm.subject.uid == null ? mainHttp + "subject/save.html" : mainHttp + "subject/update.html";
			$.ajax({
				type: "POST",
			    url: url,
		...
		getSubject:function(subjectId){
			$.ajax({
				type : "get", 
				url : mainHttp + "subject/find/"+subjectId+".html",
				async : true,
				dataType : "json",
